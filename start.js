import BlackJack from './blackjack.js'; // Import the Blackjack class
...
document.addEventListener('DOMContentLoaded', (event) => {
  let round = new BlackJack();
let hasStand = false;
let newRoundButtonAvail = false;
let hasBet = false;
let playerCard1, playerCard2, dealerCard1, dealerCard2;
let totalPlayerHits = 3;
let totalDealerHits =3;


  // UI element references
  let betButton = document.getElementById("placeBet");
  ...
  let standButton = document.getElementById("Stand_button");

  // Event listeners for game actions (betting, hitting, standing, new round)
  ...
  betButton.addEventListener('click', () => {
    ...
    let tempAmt = parseInt(betAmount.value)
  round.startRound(tempAmt);
  //round.chips -=amount;
...
hitButton.addEventListener('click', () => {
  ...
  if(!round.endRound && !hasStand && !round.hasDealerBJ && !round.hasPlayerBJ)
  {
    ...
    let playerCardNext = round.playerHit();
    document.getElementById("handValue").innerHTML = "Card Value: " +round.getPlayerSum();
    let playerimgid = number_map[totalPlayerHits] + '_card_img';
    totalPlayerHits++;
...
newRoundButton.addEventListener('click', () => {
  ...
  document.getElementById('second_card_image').src = 'PNG-cards-1.3\\face_down_card.jpg';
  // Repeat for all additional card images

  // Hide additional cards
  let diffDealer = totalDealerHits -3;
  let diffPlayer = totalPlayerHits -3 ;
  for(let i=0;i<diffDealer;i++){
    let cleanupImgDealer =  'dealer_' + number_map[i+3] + '_card_img';
    document.getElementById(cleanupImgDealer).style.display = 'none';
  }


  for(let i=0;i<diffPlayer;i++){
    let cleanupImgPlayer =  number_map[i+3] + '_card_img';
  ...
  document.getElementById('dealerValue').innerHTML = 'Dealer Hand Value: ';

  // Reset game state
  let numb = round.getchips();
  round = new BlackJack(numb);
  hasBet = false;
  hasStand=false;
...
standButton.addEventListener('click', () => {
  ...
  if(!hasStand){
   ...
   let dealerSumValue = Array.isArray(round.dealerSum) ? round.dealerSum[1] : round.dealerSum;
   document.getElementById('dealerValue').innerHTML = 'Dealer Value: ' + dealerSumValue;

   //dealer keeps hitting until gets up to 17
   while (dealerSumValue <= 16) {

    let dealercardnext = round.dealerHit();
    dealerSumValue = Array.isArray(round.dealerSum) ? round.dealerSum[1] : round.dealerSum;
    

    document.getElementById('dealerValue').innerHTML = 'Dealer Value: ' + dealerSumValue;
    let cardImgId = 'dealer_' + number_map[totalDealerHits] + '_card_img';
